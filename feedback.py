# Section: 004
# Description: Creating a new excel spreadsheet with organiezd data. Each Student is organized into their specified class, with their name, student ID, and grade. 
...
#Finds Summary statistics using Excel functions
    ...
    sheet["H2"] = f"=MAX(D2:D{sheet.max_row})"

    sheet["G3"] = "Lowest Grade"
    sheet["H3"] = f"=MIN(D2:D{sheet.max_row})"

    sheet["G4"] = "Mean Grade"
    sheet["H4"] = f"=AVERAGE(D2:D{sheet.max_row})"

    sheet["G5"] = "Median Grade"
    sheet["H5"] = f"=MEDIAN(D2:D{sheet.max_row})"

    sheet["G6"] = "Number of students in the class"
    sheet["H6"] = f"=COUNT(D2:D{sheet.max_row})"
